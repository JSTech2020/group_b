...
import originalImage from './images/Hargid.jpg'
import './Puzzle.css'
...
class  Puzzle extends Component{
    ...
    componentDidMount() {
        const pieces = [...Array(9)]
            .map((_, i) => (
                {
                    img: `hargid_${('0' + (i + 1)).substr(-2)}.jpg`,
                    order: i,
                    board: 'shuffled'
                }
            ));
    ...
    renderPieceContainer(piece, index, boardName) {
        return (
            <li key={index}
                ...
                onDrop={event => this.handleDrop(event,index,boardName)}>
                {piece && <img
                    draggable
                    onDragStart={(event => this.handleDragStart(event,piece.order))}
